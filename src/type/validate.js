...
function validateTypes(context: SchemaValidationContext): void {
  ...
  Object.keys(typeMap).forEach(typeName => {
    ...
    if (isObjectType(type)) {
      ...
      // Ensure Unions contain valid member types.
    ...
    if (!isObjectType(memberType)) {
      context.reportError(
        `Union type ${union.name} can only include Object types, ` +
          `it cannot contain ${String(memberType)}.`,
...
function validateEnumValues(
  ...
  const includedValueName = Object.create(null);
  enumValues.forEach(enumValue => {
    const valueName = enumValue.name;

    // Ensure no duplicates.
    if (includedValueName[valueName]) {
      context.reportError(
        `Enum type ${enumType.name} can include value ${valueName} ` +
          'only once.',
        // TODO: find AST nodes.
      );
    }
    includedValueName[valueName] = true;

    // Ensure valid name.
    validateName(context, enumValue);
    if (valueName === 'true' || valueName === 'false' || valueName === 'null') {
      context.reportError(
        `Enum type ${enumType.name} cannot contain value: ${valueName}.`,
        enumValue.astNode,
      );
    }
  });
